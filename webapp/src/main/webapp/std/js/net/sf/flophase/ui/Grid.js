...
define(["dojo/dom",
    ...
    return {
        ...
        onAccountUpdate: function(account) {
            var $accountHeaders = $('#account_'+account.key+', #accountbal_'+account.key);
            $accountHeaders.click(function(key) {return function(event) {
                    app.showEditAccount(
                            event,
                            this.id,
                            key);
                };}(account.key));
            $accountHeaders.text(account.name);
        ...
        onEntryEdit: function onEntryEdit(entry, acctKey, xactionKey) {
            ...
            $entryInput = $(entryInput.domNode);
            $entryInput.removeClass('positive');
            $entryInput.removeClass('negative');
            if (entry.amount < 0) {
            	$entryInput.addClass('negative');
        	} else if (entry.amount > 0) {
        		$entryInput.addClass('positive');
        	}
        ...
        onBalanceUpdate: function(transactions, account) {
            for (var i in transactions) {
                ...
                if (transactions[i].balances) {
                    ...
                    if (account) {
                        $xactionBalanceCell = $('#balance_'+account.key+'_'+transactions[i].details.key);
                        balance = dojo.currency.format(transactions[i].balances[account.key], {currency:'USD'});
                        $xactionBalanceCell.text(balance);
                        ...
                        for (var j in transactions[i].balances) {
                            $xactionBalanceCell = $('#balance_'+j+'_'+transactions[i].details.key);
                            balance = dojo.currency.format(transactions[i].balances[j], {currency:'USD'});
                            $xactionBalanceCell.text(balance);
                            
                            //get the account for the thresholds
        ...
        onTransactionUpdate: function(xaction) {
        	...
        	if (xaction.details.date >= dateRange.start && xaction.details.date < dateRange.end) {
	            ...
	            var $xactionNameCell = $('#name_'+xaction.details.key);
	            $xactionNameCell.text(xaction.details.name);
